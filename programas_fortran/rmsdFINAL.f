...
C****LA Kb ESTA EN ANGSTROMS CUADRADO
      ...
      OPEN(25, FILE='dim')
      do i=1,icontmax
      READ(21,111,end=100) cardA(1:30),r1ini(i,1),r1ini(i,2)
     $,r1ini(i,3),xx,bfactor(i)
C        write(22,222) r1ini(i,1),r1ini(i,2),r1ini(i,3)
        icont=icont+1
        write(11,113) cardA
c        write(81,114) xx,bfactor(i)
c        write(25,*) icont
      enddo
100   continue
      close(21)
      ...
      icont2=0
      do i=1,icontmax
      READ(21,111,end=200) cardB(1:30),r2ini(i,1),r2ini(i,2)
     $,r2ini(i,3),xx2,bfactor2(i) 
C       write(22,222) r2ini(i,1),r2ini(i,2),r2ini(i,3)
        icont2=icont2+1
        write(12,113) cardB
      enddo
200   continue
      ...
      if(icont.ne.icont2) write(99,*)"distinto n� de residuos"
...
C*****************************************************
      ...
      do j=1,icont
        read(11,113) cardA(1:30)
         do kk=1,3
            r1(j,kk)=r1ini(j,kk)-cm1(kk)
         enddo
        write(13,111)cardA,(r1(j,kk),kk=1,3),xx,bfactor(j)
      enddo 
      CLOSE(11)
C*****************************************************
...
c
      ...
      do j = 1,icont 
        read(12,113) cardB(1:30)
         ...
         r2(j,2) = yrot
         r2(j,3) = zrot
c        read(12,113) cardB(1:31)
        write(14,111) cardB,(r2(j,kk),kk=1,3),xx2,bfactor2(j)
      enddo
      CLOSE(12)
C***************************************************
...
111   FORMAT(A30,3(F8.3),F6.2,F6.2)
112   FORMAT(3(F8.3))
113   FORMAT(A30)
